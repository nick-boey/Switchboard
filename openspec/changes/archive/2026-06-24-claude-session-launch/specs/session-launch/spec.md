...
survives the request and the official Claude mobile app can drive it. The launch command MUST be
passed as argv (never a shell line), MUST rely on the host's existing `claude` login (no per-session
pairing UI), and MUST require an existing worktree.
...
- **THEN** a detached tmux session is created with its working directory set to that worktree's
  checkout path, running `claude --remote-control`
