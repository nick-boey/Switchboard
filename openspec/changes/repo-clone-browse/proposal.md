...
- Add a **Git service** that performs a **bare clone** of a chosen repo into
  `~/.switchboard/repos/<owner>/<repo>/.bare`, and lists already-cloned repos from disk.
...
- `repo-clone`: bare-clone a chosen repo to `~/.switchboard/repos/<owner>/<repo>/.bare`, list cloned repos, with
...
- Filesystem: `~/.switchboard/repos/<owner>/<repo>/.bare`; `~/.switchboard` credential-helper config.
