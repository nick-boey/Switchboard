...
**Archive disposition (decided 2026-06-23 — archive intentionally deferred).** Implementation is
complete and all checkpoints have passed, but the archive's prototype-reconcile is **deliberately
held**: the three screen sketches (`worktrees`, `new-repository`, `settings`) are the input that the
active downstream changes refine into real UI — `repo-clone-browse` and `worktree-management` both
state they "refine the `ui-prototypes-mvp` prototypes," and `claude-session-launch` consumes the
plug/session model. They stay **`open`** until those changes consume/supersede them, at which point
each resolves to `delete — superseded by <change>`. The `design-language` gallery is already
superseded (its primitives are promoted to production `src/ui/*` stories + the `Foundations/Theme`
gallery) and will resolve to `delete` at archive. `kit.tsx`/`hub.tsx` are deleted with the folder.
