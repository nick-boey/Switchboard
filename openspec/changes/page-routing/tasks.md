...
- [ ] 6.2 **Archive gate (production SPA fallback, see `dependencies.md`):** before archiving, confirm that whatever serves the *built* SPA in production returns `index.html` as a history fallback for unknown non-API paths, and that it is verified there — otherwise deep-linked/reloaded clean paths 404 in production even though they pass under Vite. `runtime-cli-docker` is the intended owner but currently carries **no** SPA-static-serving / history-fallback spec or task, so this obligation is presently **unmet**. Do not archive page-routing until that fallback exists and is verified (add the `depends-on: [runtime-cli-docker]` edge if its scope is confirmed to cover it; otherwise resolve the owner first).
