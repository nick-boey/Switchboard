 ...
 * 3. `tailscale up` reading the **mounted auth-key secret** by file reference
 *    (`--auth-key=file:<path>`), so the key never lands in argv/logs, with a stable hostname;
 ...
 *    http://127.0.0.1:<servePort>` — a non-zero `serve` exit FAILS the bring-up (no external HTTPS
 *    ingress means no healthy runtime).
 *
 * Both `tailscaled` (a child) and the server (via `superviseServer`) are supervised CONCURRENTLY for
 * the container's lifetime: an unexpected `tailscaled` exit closes the server and exits non-zero
 * (never report healthy with a dead Tailscale ingress). A shutdown signal closes the server and is
 * **forwarded to `tailscaled`**.
...
export interface DockerBringUpOptions {
  ...
  servePort: number;
  /**
   * Filesystem path to the MOUNTED Tailscale auth-key secret. Passed to `tailscale up` via the
   * `--auth-key=file:<path>` form so the key stays file-resident and never appears in process argv,
   * logs, or image layers (the no-leak boundary). The CLI never reads the key value itself.
   */
  authKeyFile: string;
...
export async function runDockerBringUp(options: DockerBringUpOptions): Promise<number> {
  const { runner, start, shutdownSignal, logger, servePort, authKeyFile } = options;
  ...
  // 3. tailscale up reading the mounted auth-key secret by FILE REFERENCE (`file:` form) so the key
  // stays file-resident and never appears in argv/logs (no-leak).
  ...
  // Supervise the server through an internal abort that closes it on EITHER an external shutdown OR
  // an unexpected `tailscaled` exit — so a dead Tailscale ingress never leaves a "healthy" server.
  const serverShutdown = new AbortController();
  const requestServerStop = (): void => serverShutdown.abort();
  if (shutdownSignal.aborted) requestServerStop();
  else shutdownSignal.addEventListener('abort', requestServerStop, { once: true });

  // Race tailscaled liveness against the server supervisor: if tailscaled exits while we did NOT
  // request shutdown, it is a fault — tear the server down and fail the bring-up.
  let tailscaledFaulted = false;
  void tailscaled.exited.then((tsCode) => {
    if (shutdownSignal.aborted) return; // an expected exit during graceful shutdown
    tailscaledFaulted = true;
    logger.error('tailscaled exited unexpectedly; closing the server', { code: tsCode });
    requestServerStop();
  });
  ...
  try {
    supervised = await superviseServer({
      start,
      shutdownSignal: serverShutdown.signal,
      logger,
      sleep: options.sleep,
      ...
      onListening: async (handle) => {
        ...
        served = true;
        const result = await runner.run('tailscale', [
          'serve',
          '--bg',
          '--https=443',
          `http://127.0.0.1:${servePort}`,
        ]);
        // `tailscale serve` is the ONLY external HTTPS ingress: if it fails, the runtime is not
        // healthy. Tear the server down and fail fast rather than parking on it (Decision 6).
        if (result.code !== 0) {
          logger.error('tailscale serve failed; no external ingress configured', {
            code: result.code,
          });
          await handle.close();
          throw new Error(`tailscale serve failed (exit ${result.code})`);
        }
      },
    });
  } finally {
    // Whatever path we leave on (clean shutdown, serve failure, or tailscaled fault), signal and
    // reap tailscaled before returning.
    forward();
    await tailscaled.exited;
  }

  // A dead tailscaled is a fault even though the server closed cleanly under the internal abort.
  if (tailscaledFaulted) return 1;
  return supervised;
}
